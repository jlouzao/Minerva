// Compras.cpp: define las rutinas de inicialización del archivo DLL.
//
...
static AFX_EXTENSION_MODULE ComprasDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("Inicializando Compras.DLL\n");
		
		// Inicialización única del archivo DLL de extensión
		if (!AfxInitExtensionModule(ComprasDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(ComprasDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("Finalizando Compras.DLL\n");

		// Finalizar la biblioteca antes de llamar a los destructores
		AfxTermExtensionModule(ComprasDLL);
	}
	return 1;   // aceptar
}
