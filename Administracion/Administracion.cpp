// Administracion.cpp: define las rutinas de inicialización del archivo DLL.
//
...
static AFX_EXTENSION_MODULE AdministracionDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("Inicializando Administracion.DLL\n");
		
		// Inicialización única del archivo DLL de extensión
		if (!AfxInitExtensionModule(AdministracionDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(AdministracionDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("Finalizando Administracion.DLL\n");

		// Finalizar la biblioteca antes de llamar a los destructores
		AfxTermExtensionModule(AdministracionDLL);
	}
	return 1;   // aceptar
}
