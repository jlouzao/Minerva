// GestionCuentas.cpp: define las rutinas de inicialización del archivo DLL.
//
...
static AFX_EXTENSION_MODULE GestionCuentasDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("Inicializando GestionCuentas.DLL\n");
		
		// Inicialización única del archivo DLL de extensión
		if (!AfxInitExtensionModule(GestionCuentasDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(GestionCuentasDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("Finalizando GestionCuentas.DLL\n");

		// Finalizar la biblioteca antes de llamar a los destructores
		AfxTermExtensionModule(GestionCuentasDLL);
	}
	return 1;   // aceptar
}
