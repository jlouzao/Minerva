// Cobros.cpp: define las rutinas de inicialización del archivo DLL.
//
...
static AFX_EXTENSION_MODULE CobrosDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("Inicializando Cobros.DLL\n");
		
		// Inicialización única del archivo DLL de extensión
		if (!AfxInitExtensionModule(CobrosDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(CobrosDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("Finalizando Cobros.DLL\n");

		// Finalizar la biblioteca antes de llamar a los destructores
		AfxTermExtensionModule(CobrosDLL);
	}
	return 1;   // aceptar
}
