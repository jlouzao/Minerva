// Almacen.cpp: define las rutinas de inicialización del archivo DLL.
//
...
static AFX_EXTENSION_MODULE AlmacenDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("Inicializando Almacen.DLL\n");
		
		// Inicialización única del archivo DLL de extensión
		if (!AfxInitExtensionModule(AlmacenDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(AlmacenDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("Finalizando Almacen.DLL\n");

		// Finalizar la biblioteca antes de llamar a los destructores
		AfxTermExtensionModule(AlmacenDLL);
	}
	return 1;   // aceptar
}
